            ...
            try {
                ...
                foreach ($vidpidKey in $vidpidKeys) {
                    ...
                    foreach ($instanceKey in $instanceKeys) {
                        ...
                        # Defer: trusted HID vendor but OS has not written Class or ClassGUID yet.
                        # The device node is still being created; re-evaluate on the next poll
                        # (250 ms) rather than transiently blocking a legitimate keyboard or mouse.
                        # Non-HID devices with trusted VIDs (e.g. Apple iPhones VID_05AC, which
                        # present as class "Image"/"WPD") will have their class set within one or
                        # two polls and will fall through to the block path at that point.
                        if ($isHIDVendor -and (-not $regClass) -and (-not $regClassGuid)) {
                            [void]$knownInstanceIds.Remove($instanceId)
                            Write-WatcherLog "DEFERRED $vidpid - trusted HID vendor, class not yet written; re-evaluating" -Level "INFO"
                            continue
                        }

                        # If vendor is in the HID list but Class/ClassGUID confirm a non-HID
                        # class (e.g. Apple iPhone: VID_05AC, class=Image/WPD), fall through
                        # to the whitelist/block decision below.
